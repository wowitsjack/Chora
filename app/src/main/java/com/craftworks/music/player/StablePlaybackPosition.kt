...
internal fun stablePlaybackPosition(
    ...
    val projected = if (isPlaying) previous + elapsedMs.coerceAtLeast(0L) else previous

    val resolved = when {
        allowDiscontinuity -> reported
        reportedPositionMs < 0L -> projected
        reported + POSITION_REWIND_TOLERANCE_MS < previous -> projected
        isPlaying -> maxOf(reported, projected)
        else -> reported
    }
